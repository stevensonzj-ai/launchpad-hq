...
export const surferSeoTutorial: PlatformTutorialData = {
  ...
  whatItIs: [
    "Surfer is a writing tool for people trying to get found in search: it compares what you are writing against the pages already ranking for your phrase and shows what they cover that you do not.",
    ...
    "It will also draft the whole article for you from a phrase and some instructions. Whether you write it yourself or let it draft, the same scoring panel sits beside you.",
    "One cosmetic note: the parent company's rename shows up in browser tabs as \"Positive Surfer.\" The product is still called Surfer and still lives at surferseo.com.",
  ],
  ...
  beforeYouStart: [
    ...
    "Two pieces of Surfer cost nothing and want no subscription. Keyword Surfer, a browser extension Surfer's own page calls \"100% free. Forever.\", shows search volumes inside your normal Google results; the Chrome Web Store lists it as last updated January 2026 with around 600,000 users. The AI Humanizer rewrites up to 500 words with no account, or 1,000 once you register without a plan — English only, as the vendor states it today.",
    "The trial runs once: Surfer's help centre says it is for new accounts only, and a card already registered with Surfer makes you ineligible, so you cannot restart it under a new email with the same card.",
  ...
  security: [
    { kind: 'text', text: "Surfer is run by Surfer sp. z o.o., a company in Wrocław, Poland, so its customer privacy policy — last updated 23 January 2026 — is written to European privacy law. Most of what you put in is marketing copy about your own business, which makes this lower-stakes than a general chatbot." },
    { kind: 'list', label: 'Worth knowing before you paste your site into it', items: [
      ...
      "Surfer's help centre warns that after you cancel, the account expires and stored data goes with it — export before your last paid day, to plain text, HTML or Markdown.",
    ] },
  ],
  ...
  triad: {
    bestAt: [
      ...
      "Getting a competent first draft out of a single phrase when the blank page is the problem",
    ],
    okayAt: [
  ...
  starterActions: [
    {
      ...
      whyHere:
        "The guidelines Surfer builds inside its paid editor rest on this same search-volume layer, and this is the one place you can inspect it without a login. Tested against Semrush, whose keyword numbers live inside a dashboard you have to go to: here you judge the data first, in a page you already trust.",
    },
    {
      ...
      whyHere:
        "This goes in the \"context & instructions\" box on the Content Editor's setup screen, before \"Generate content.\" Surfer builds its guidelines from the pages currently ranking, so left alone it drafts toward your competitors' average; its own documentation says this box takes free-text instructions plus up to five source URLs, and it is the only point before generation where you can contradict that average. Tested against ChatGPT, which has not read the ten pages ranking for your phrase: there is no average for your instructions to push against.",
      tweak:
    ...
    },
    {
      ...
      whyHere:
        "You select a paragraph in a Surfer document and click \"Ask Surfy.\" Surfy exists only on selected text, and Surfer's docs say it runs on its own separate pool of credits (Surfer's documentation calls them tokens) rather than the allowance a full article generation spends, so small fixes are metered apart from big ones. Tested against ChatGPT: the same rewrite there means pasting back to find out whether the score moved.",
    },
    {
      ...
      whyHere:
        "Add this as a tracked prompt in the AI Tracker; Surfer's docs say tracked prompts refresh daily. Surfer reports two different things about that answer — whether your brand was named, and whether your page was among the sources the model read — and its documentation flags that these often disagree. Tested against a conventional rank tracker: a Google result either lists your page or it does not; an AI answer can cite you without naming you, or name you without citing you. The entry-level plan currently tracks zero prompts, so this one needs Standard or above.",
    },
    {
      ...
      whyHere:
        "Surfer counts this against the same monthly \"documents\" allowance as a fresh article — currently 120 a month on the entry plan — so auditing an old page costs what creating a new one does; check your best pages first, while the allowance is full. Tested against Google Search Console, free and already in most people's hands: it tells you a page lost position, not which subtopics the pages that overtook you cover.",
    },
  ],

  pitfalls: [
    "The pages Surfer compared you to can move, so a score is a reading from one day rather than a standing.",
    "Writing to the term list mechanically shows: the panel gives a suggested count range for each term, and hitting the top of every range is how Surfer-written pages come to read like Surfer-written pages. Treat it as subjects to cover, not a quota.",
    ...
    "The interface itself is available in English, Polish and French only, which is separate from which languages it can analyse or write in.",
    "Surfer's plan names are still settling after the acquisition: its own August 2026 changelog refers to tiers the live pricing page no longer lists. Every price and limit here is the published position on the day it was checked.",
  ],
