...
export const activepiecesTutorial: PlatformTutorialData = {
  ...
  beforeYouStart: [
    "**The free cloud plan is a plan, not a trial.** No card, unlimited flows, one user, and currently around 100 credits a day — a fresh batch each morning rather than a monthly bucket. Since a run costs one credit however many steps it has, that is roughly a hundred flow runs a day before anything stops.",
  ...
  security: [
    {
      kind: "text",
      text: "Where your connected logins physically sit is a decision you actually get to make here, and it is worth making on purpose. The Activepieces cloud currently offers a single data region, Germany, so everything your flows touch is processed there. Its security docs say credentials are held under 256-bit encryption with no interface to read them back out, and that sensitive values are stripped out of run logs. Choose to run it on your own machine instead and every one of those protections becomes something you are responsible for.",
    },
    {
      kind: "list",
      label: "Worth knowing before you switch a flow on",
      items: [
        "The free plan is one person. There is no way to let a colleague in without handing over your own login — adding people is what the paid plans are for, and single sign-on and audit logs sit in the paid part on cloud and self-hosted alike.",
  ...
  triad: {
    bestAt: [
      "Flows with a lot of steps, since one run costs one credit whether it has three steps or thirty",
  ...
  starterActions: [
    {
      ...
      whyHere:
        "Three steps, one credit — and the allowance refills tomorrow morning, so a flow you misconfigured costs you a day. On Zapier that same flow spends a separate task per step out of a hundred a month; n8n prices by the run too, but has no permanent free cloud plan to do it on.",
      tweak:
    ...
    },
    {
      ...
      whyHere:
        "Activepieces posts what the thinking costs before you run it — 2 credits on a fast model, 10 on a smarter one, 20 on a frontier one — so you can weigh a daily habit against a hundred-a-day allowance in your head. Zapier sells its AI features as separate products with their own plans, and n8n expects you to bring your own model account rather than quoting you a rate.",
      tweak:
    ...
    },
    {
      ...
      whyHere:
        "Around 760 pieces, each MIT-licensed and published as its own package, most of them written by people outside the company. On Zapier a missing integration is a request you file with a company; n8n is open too, but ships its own connectors under a licence that stops you reusing them freely.",
      tweak:
        "If the piece you want does not exist, the framework for writing one is public. That is a project rather than an afternoon, but it is not a closed door.",
    },
  ],

  pitfalls: [
    "**What you self-host is the automation core, not the cloud product minus the bill.** Community Edition leaves out projects, single sign-on, audit logs and Git Sync. For one person automating their own work that is no loss; for a company it is the entire reason the paid tiers exist.",
    "**AI steps are the only thing that breaks the one-credit-per-run rule**, and they break it by up to twenty times. Price a single run before you schedule it hourly.",
    "**Credits refresh, they don't accumulate.** A quiet week does not buy you a busy Monday.",
